...
class StudentLearningAgent:
    ...
    def learning_round(self, round_num: int, total_rounds: int, topic: str, 
        ...
        decision = self._make_decision(context, round_num)
    ...
    def _make_decision(self, context: Dict, round_num: int) -> Dict:
        ...
        if 'self_study' in response_lower or 'self study' in response_lower or 'review' in response_lower:
            action = 'self_study'
    ...
    def _execute_self_study(self, context: Dict, round_num: int) -> Dict:
        ...
        content = self.agent.extract_content(response, 'content')
        if not content:
            content = response[:50]
    ...
    def _execute_ask_teacher(self, context: Dict, round_num: int, teacher_agent) -> Dict:
        ...
        question = self.agent.extract_content(response, 'content')
        if not question:
